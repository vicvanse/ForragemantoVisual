/** SHA-256 síncrono (subset) para sementes reprodutíveis no browser. */
function sha256Bytes(message: string): Uint8Array {
  const enc = new TextEncoder();
  const data = enc.encode(message);
  // Web Crypto is async; use a compact sync implementation via SubtleCrypto polyfill pattern
  // For Next.js client bundles, we use a minimal pure-JS SHA-256.
...
function sha256Pure(data: Uint8Array): Uint8Array {
  ...
  view.setUint32(withOne.length - 4, ml, false);
...
export function pickSessionIndex(seed: string, count: number): number {
  const digest = sha256Bytes(`session_pick|${seed}`);
  const val =
